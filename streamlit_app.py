            ...
            if st.button("Run"):
                if len(shift_keys) < 2 or len(shift_keys) > len(text):
                    ...
                    try:
                        ...
                        st.markdown(f"<h4 style='color:#2e7dba'>{enc_title}</h4>", unsafe_allow_html=True)
                        st.markdown(
                            f"<div style='background:#f6f8fa;border-radius:8px;padding:12px;margin-bottom:10px;overflow-x:auto;'>"
                            f"<pre style='font-size:14px;line-height:1.5'>{enc_report}</pre></div>",
                            unsafe_allow_html=True
                        )
                        st.markdown(f"<h4 style='color:#2e7dba'>{dec_title}</h4>", unsafe_allow_html=True)
                        st.markdown(
                            f"<div style='background:#f6f8fa;border-radius:8px;padding:12px;margin-bottom:10px;overflow-x:auto;'>"
                            f"<pre style='font-size:14px;line-height:1.5'>{dec_report}</pre></div>",
                            unsafe_allow_html=True
                        )
                        st.markdown("---")
                        st.markdown(
